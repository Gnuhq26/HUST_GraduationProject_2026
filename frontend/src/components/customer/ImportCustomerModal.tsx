...
export default function ImportCustomerModal({ onClose, onSuccess }: Props) {
  ...
  const handlePreview = async () => {
    ...
    setError('');
    try {
      const data = await customersService.previewImport(file);
  ...
  const handleCommit = async () => {
    ...
    setError('');
    try {
      const result = await customersService.commitImport(file);
      setCommitResult(result);
      setStep('result');
    } catch (err: unknown) {
      const e = err as { response?: { data?: { message?: string } } };
      setError(e.response?.data?.message || 'Lỗi import khách hàng');
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="fixed inset-0 backdrop-blur-sm bg-blacky-950/30 flex items-center justify-center z-50 p-4">
      <div className="bg-basic-white rounded-2xl border border-basic-border max-w-3xl w-full max-h-[90vh] flex flex-col shadow-lg">
        {/* Header */}
        <div className="px-6 py-5 border-b border-yellowfish-400  flex items-center justify-between shrink-0">
          <div>
            <h2 className="text-xl font-bold text-blacky-950">Nhập danh sách khách hàng từ Excel</h2>
            ...
            <div className="space-y-5">
              <div className="bg-bluesh-50 border border-bluesh-800/50 rounded-xl p-4">
                ...
                <p className="text-sm text-blacky-500 mb-3">
                  Tải file Excel mẫu, điền thông tin khách hàng theo đúng cấu trúc, sau đó upload lại.
                </p>
                <button
                  type="button"
                  ...
                  <table className="w-full text-sm">
                    <thead className="bg-bluesh-800 sticky top-0">
                      <tr>
                        ...
                        <th className="px-3 py-2 text-left text-xs font-medium text-basic-white">Tên khách hàng</th>
                        <th className="px-3 py-2 text-left text-xs font-medium text-basic-white">SĐT</th>
                        <th className="px-3 py-2 text-left text-xs font-medium text-basic-white">Địa chỉ</th>
                        <th className="px-3 py-2 text-center text-xs font-medium text-basic-white">Hành động</th>
                    ...
                    <tbody className="divide-y divide-basic-border">
                      {previewData.rows.map((row, idx) => (
                        ...
                        >
                          <td className="px-3 py-2 text-blacky-500">{row.rowNumber}</td>
                          <td className="px-3 py-2 font-medium text-blacky-700">{row.customerName}</td>
                          <td className="px-3 py-2 text-blacky-700">{row.phone || '—'}</td>
                          <td className="px-3 py-2 text-blacky-700">{row.address || '—'}</td>
                          <td className="px-3 py-2 text-center">
                            {row.errors?.length === 0 && (
                              <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${
                                row.action === 'UPDATE' ? 'bg-bluesh-50 text-bluesh-800' : 'bg-accent-green/10 text-accent-green'
                              }`}>
          ...
          {step === 'result' && commitResult && (
            <div className="space-y-4">
              <div className="flex flex-col items-center py-6 gap-3">
                <div className="w-16 h-16 rounded-full border border-accent-green bg-accent-green/10 flex items-center justify-center">
                  <CheckCircle className="w-8 h-8 text-accent-green" />
                </div>
                <h3 className="text-lg font-semibold text-blacky-950">Nhập thành công!</h3>
              </div>
              <div className="grid grid-cols-3 gap-4">
